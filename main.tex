...
\ShortHeadings{Imitation Learning for Accelerating Self-Consistent Methods}{Meil\u{a} and Jordan}
\firstpageno{1}
...
Self consistent methods have emerged in various different fields of science and mathematics. These methods gives means to iteratively reach a fixed value by making the result a function of itself. These methods are usually fixed point iteration methods which is described as given in the equation below.
\[
x_{n+1} = f(x_n),  n = 0,1,2,3...
\] 
The fixed point algorithm will iterate until reaching a steady state at which point each iteration will be self-consistent[ref] and $f(x)=x$.  
...
The chemical data consists of the electronic structure of a set of various unique molecules, each with several distorted geometries. The bond lengths and bond angles were distorted with a random uniform distribution of +-0.2 \AA\ and +-10\textsuperscript{o}. The geometries have a free rotation on the along the first carbon-carbon bond. The dataset was generated using rejection sampling, meaning that once the geometry is distorted it only is accepted given the condition that the distortion of all the bond lengths and angles are within the given range and that there are no contact closer than 3 \AA\ between non-bonded atoms. All the geometries are placed in different fixed electric field environments in X, Y and Z direction. 

The dataset is composed of different derivations of pentylene, as shown in figure \ref{fig:molecule}. The molecule is singly substituted with flourine (-F), hydroxyl (-OH) or amine (-NH2) group. Hence the dataset has a set off geometries where each carbon is substituted with each of the 3 subsituents leading to a total of 16 unique molecules including the molecule without any subsituents.  

\begin{figure}[h!]
  \caption{The chemical structure of Pentylene, here R = H, F, OH or NH2}
    \label{fig:molecule}
